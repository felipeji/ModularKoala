...
version = 1.1 # agregado a mano para que no de error

# -----------------------------------------------------------------------------
# Load all PyKOALA tasks   / Import PyKOALA 
# -----------------------------------------------------------------------------
from koala.io import list_fits_files_in_folder
from koala_rss import KOALA_RSS
"""
from automatic_scripts.automatic_calibration_night import automatic_calibration_night
from koala.plot_plot import plot_plot
from koala.onedspec import fluxes
from koala.onedspec import dfluxes
from koala.onedspec import gauss
"""
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
...
path = "/Users/felipe/DataCentral/KOALA/sample_RSS/"
...
objects_auto = automatic_calibration_night(path=path_red, auto=True)
...
# # objects_auto['HD60753_385R_20180227'] : Python object with calibration star HD60753
# # objects_auto['Hilt600_385R_20180227'] : Python object with calibration star Hiltner 600
...
# La siguiente rutina me da error al entrar en el if de "cal_from_calibrated_starcubes = True"
"""
automatic_calibration_night(path=path_red, auto=True, 
                            list_of_objects = objects_auto,
                            #star_list =["Hilt600"],
                            grating="385R",
                            date="20180227",
                            throughput_2D_file=throughput_2D_file,
...
file_in   = os.path.join(path_red, "27feb20031red.fits")
...
"""
file_med  = os.path.join(path_red,"27feb20031red_TCWXU____.fits")
...
# #
"""
from koala.constants import fibres_best_sky_100
sky1 = KOALA_RSS(os.path.join(path_red,"27feb20030red.fits"),
                #save_rss_to_fits_file="auto",
                apply_throughput=True, 
                throughput_2D_file=throughput_2D_file,
                correct_ccd_defects = True, 
...
"""
file_med2 = os.path.join(path_red,"27feb20031red_TCWXUS___.fits")
...
Al ejecutar esto me da error el map de RSS:
    
File "/Users/felipe/DataCentral/KOALA/koala-modular_version/src/koala/RSS.py", line 3831, in RSS_map
  if log == False: norm = colors.Normalize()
    
File "/Users/felipe/DataCentral/KOALA/koala-modular_version/src/koala/RSS.py", line 2459, in find_sky_fibres
    if plot: self.RSS_map(self.integrated_fibre, None, self.sky_fibres, title=" - Sky Spaxels")    

creo que esta cogiendo mal una de las variables. Solucion asignar las variables de manera explicita y no de forma posicional
"""
...
#Lo siguiente da error porque no existe el fichero. Creo que viene del paso anterior que no corre
file_out  = os.path.join(path_red,"27feb20031red_TCWXUS_NR.fits")
